...
\paragraph{}Na análise das técnicas de otimização de Fibonacci, foi escolhida a função ‘\textit{fib}’. Isso se deve pelo fato dela ser a parte mais importante do programa.
...
\paragraph{}Em relação ao nível 2 de otimização, se comparado com o nível 1, é possível perceber que foi adicionado uma comparação inicial do valor passado à função ‘\textit{fib}’, por meio da instrução ‘\textit{icmp}’. Essa comparação garante que se na primeira vez que a função ‘\textit{fib}’ for chamada, for passado um valor menor ou igual a 2, a função não irá executar instruções da recursão, aumentando a eficiência do programa.
...
\paragraph{}Para a análise das técnicas de otimização de \textit{Private}, foi escolhida a função \textit{main}. Isso se deve pelo fato desta função apresentar um tipo específico de otimização pertinente ao programa \textit{Private}.
...
\paragraph{}Como é possível perceber, no nível 1 de otimização, se comparado com a versão sem otimização, a função ‘\textit{main}’ deixou de executar as instruções de inicialização de uma função. Isso se deve pelo fato de que na ‘\textit{main}’, nada é executado, portanto essas instruções são desnecessárias.
